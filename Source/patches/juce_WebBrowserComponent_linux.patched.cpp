...
private:
    ...
    bool loadSymbols (DynamicLibrary& lib, SymbolBinding<FuncPtr> binding)
    {
        ...
        std::cerr << "OrbitLooper: Failed to load symbol: " << binding.name << std::endl;
        return false;
    }
    ...
    bool loadGtkSymbols()
    {
        if (gtkLib.getNativeHandle() == nullptr)
            gtkLib.open ("libgtk-3.so.0");
    ...
    {
        if (glib.getNativeHandle() == nullptr)
            glib.open ("libglib-2.0.so.0");
    ...
    {
        std::cerr << "OrbitLooper DEBUG: Trying combo [" << names.webkitLib << ", " << names.jsLib << ", " << names.soupLib << "]" << std::endl;

        if (! webkitLib.open (names.webkitLib))
        {
            std::cerr << "  -> libwebkit open failed" << std::endl;
            return false;
        }

        if (! jsLib.open (names.jsLib))
        {
            std::cerr << "  -> libjavascriptcore open failed" << std::endl;
            webkitLib.close();
            return false;
        }

        if (! soupLib.open (names.soupLib))
        {
            std::cerr << "  -> libsoup open failed" << std::endl;
            webkitLib.close();
            jsLib.close();
            return false;
        }

        std::cerr << "OrbitLooper: WebKit libraries found and opened." << std::endl;
        return true;
    }
    ...
    const bool webKitIsAvailable =    (   openWebKitAndDependencyLibraries ({ "libwebkit2gtk-4.1.so",     "libjavascriptcoregtk-4.1.so",    "libsoup-3.0.so" })
                                       || openWebKitAndDependencyLibraries ({ "libwebkit2gtk-4.1.so.0",   "libjavascriptcoregtk-4.1.so.0",  "libsoup-3.0.so.0" })
                                       || openWebKitAndDependencyLibraries ({ "libwebkit2gtk-4.0.so.37",  "libjavascriptcoregtk-4.0.so.18", "libsoup-2.4.so.1" })
                                       || openWebKitAndDependencyLibraries ({ "libwebkit2gtk-4.0.so",     "libjavascriptcoregtk-4.0.so",    "libsoup-2.4.so" })
                                       || openWebKitAndDependencyLibraries ({ "libwebkit2gtk-4.0.so.3",   "libjavascriptcoregtk-4.0.so.3",   "libsoup-2.4.so.1" })
                                       || openWebKitAndDependencyLibraries ({ "libwebkit2gtk-4.0.so.3",   "libjavascriptcoregtk-4.0.so.3",   "libsoup-2.4.so.0" })
                                       || openWebKitAndDependencyLibraries ({ "libwebkit2gtk-4.0.so.0",   "libjavascriptcoregtk-4.0.so.0",  "libsoup-2.4.so.1" }))
                                    && loadWebkitSymbols()
                                   && loadGtkSymbols()
                                   && loadJsLibSymbols()
                                   && loadSoupLibSymbols()
...
public:
    ...
    int entry()
    {
        ...
        {
            std::cerr << "OrbitLooper FATAL: WebKit is not available on this system!" << std::endl;
            return 1;
        }

        // webkit2gtk crashes when using the wayland backend embedded into an x11 window
        // unless explicitly handled. We now allow overriding via environment or compile definition.
        const char* backend = std::getenv("GDK_BACKEND");
        if (backend == nullptr)
        {
           #ifdef ORBITLOOPER_WAYLAND
            backend = "wayland";
           #else
            backend = "x11";
           #endif
        ...
        std::cerr << "OrbitLooper: Setting GDK_BACKEND to: " << backend << std::endl;
        wk.juce_gdk_set_allowed_backends (backend);

        std::cerr << "OrbitLooper: Calling gtk_init..." << std::endl;
        ...
        std::cerr << "OrbitLooper: Creating webviewWidget..." << std::endl;
        ...
        std::cerr << "OrbitLooper: webviewWidget created." << std::endl;
    ...
    {
        std::cerr << "[JUCE] GtkChildProcess::receiverHadError!" << std::endl;
        exit (-1);
    }
